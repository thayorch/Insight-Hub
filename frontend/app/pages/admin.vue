<template>
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="mb-8">
      <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">บันทึกประเด็นปัญหา</h1>
      <p class="mt-2 text-sm text-slate-500">กรอกประเด็นที่รวบรวมได้ (เช่น จากกลุ่มลูกช้าง) เพื่อให้ AI ประเมินความเสี่ยงและนำเข้าสู่แดชบอร์ดสรุปผลสำหรับผู้บริหาร</p>
    </div>
...
async function initMap() {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (!pickerMap) {
      pickerMap = L.map('picker-map').setView([18.8038, 98.9530], 15)
